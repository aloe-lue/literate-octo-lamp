...
list_node *create_node(char *key)
{
...
void append_node(list_node **head, char *key)
{
        if (*head == NULL) {
...
void preppend_node(list_node **head, char *key)
{
...
bool contains_node(list_node **head, char *key)
{
        list_node *temp = *head;
...
void insert_node_at(list_node **head, char *key, int idx)
{
        if (*head == NULL) {
                *head = create_node(key);
                return;
        ...
        if (idx <= 0) {
                preppend_node(head, key);
                return;
        }
        if (idx >= nodes_size(head) - 1) {
                append_node(head, key);
                return;
        }
