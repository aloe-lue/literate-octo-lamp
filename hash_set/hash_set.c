...
void hash_set_add(list_node **keys, list_node **buckets, char *key)
{
        if (key == NULL || strlen(key) == 0) {
                puts("you need key to store in the buckets\n");
                return;
        }
        if (*buckets == NULL) {
...
bool hash_set_has(list_node **buckets, char *key)
{
        if (*buckets == NULL) {
                puts("there\'s is nothing to get in bucket. \n");
                return false;
...
int hash_set_remove(list_node **keys, list_node **buckets, char *key)
{
